...
export const Sidebar = () => {
  ...
  const getRoleLabel = () => {
    ...
    return { label: 'Admin Executive Suite', color: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20' }
  }
    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-3 border-b border-slate-800/80">
        <div className="flex items-center gap-2 overflow-hidden">
          <div className="w-11 h-11 bg-slate-800/90 p-1 rounded-full border border-slate-700/60 shadow-sm flex items-center justify-center shrink-0">
            <img src={haloLogo} alt="The Halo Effect Consulting" className="w-full h-full object-contain rounded-full" />
          </div>
          {sidebarOpen && (
            <div className="flex flex-col leading-tight overflow-hidden">
              <span className="font-bold text-slate-100 text-xs tracking-wide whitespace-nowrap">
                BUSINESS OS
              </span>
              <span className="text-[10px] text-indigo-400 font-semibold tracking-wider mt-1 uppercase whitespace-nowrap">
                {displayRole} MODE
              </span>
            </div>
          )}
        ...
        <button
          ...
          className="w-8 h-8 rounded-lg bg-slate-800/60 hover:bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center transition-colors cursor-pointer shrink-0"
        >
      ...
      <div className="p-3 border-t border-slate-800/80">
        <div className={`flex items-center gap-3 p-2 rounded-xl bg-slate-800/40 border border-slate-800 ${!sidebarOpen && 'justify-center'}`}>
          ...
          {sidebarOpen && (
            <div className="flex flex-col min-w-0">
              <span className="text-xs font-semibold text-slate-200 truncate">{user?.displayName || 'Acme Executive'}</span>
