    ...
    <div className="space-y-6">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <Card className="w-full max-w-2xl p-6 space-y-4 border-slate-800 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="font-bold text-slate-100 text-sm">Create New Invoice</h3>
              <button
            ...
            <form onSubmit={handleCreateInvoice} className="space-y-4">
              ...
              <div className="space-y-3 pt-2 border-t border-slate-800">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-300">Line Items</span>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                ...
                {lineItems.map((item, index) => (
                  <div key={index} className="flex gap-2 items-center">
                    <input
                      ...
                      className="flex-1 bg-[#11141E] border border-slate-800 text-slate-100 text-xs rounded-xl p-2.5 focus:outline-none"
                      required
                    />
                    <input
                      type="number"
                      ...
                      className="w-16 bg-[#11141E] border border-slate-800 text-slate-100 text-xs rounded-xl p-2.5 focus:outline-none"
                    />
                    <input
                      type="number"
                      ...
                      className="w-28 bg-[#11141E] border border-slate-800 text-slate-100 text-xs rounded-xl p-2.5 focus:outline-none"
                    />
                    <button
                      type="button"
              ...
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-1.5 text-xs text-right">
                ...
                <div className="flex justify-between items-center text-slate-400">
                  ...
                  <input
                    ...
                    className="w-16 bg-[#181C27] border border-slate-800 text-slate-100 text-xs rounded px-2 py-0.5 text-right"
                  />
                </div>
      ...
      {selectedInvoice && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <Card className="w-full max-w-lg p-6 space-y-4 border-slate-800 shadow-2xl relative">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div>
                <h3 className="font-bold text-slate-100 text-base">{selectedInvoice.invoiceNumber}</h3>
                <p className="text-xs text-indigo-400">{selectedInvoice.clientName}</p>
              </div>
              <button
            ...
            <div className="space-y-2 text-xs">
              {selectedInvoice.lineItems?.map((item, idx) => (
                <div key={idx} className="flex justify-between py-1.5 border-b border-slate-800/60 text-slate-300">
                  <span>{item.description} ({item.quantity}x)</span>
                  <span className="font-semibold">${(item.quantity * item.unitPrice).toLocaleString()}</span>
                </div>
              ))}

              <div className="pt-3 flex justify-between font-bold text-sm text-emerald-400">
