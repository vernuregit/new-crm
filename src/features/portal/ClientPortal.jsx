...
export const ClientPortal = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-4 border-indigo-500/30 bg-gradient-to-r from-indigo-500/10 to-purple-500/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-indigo-600/20 text-indigo-400 flex items-center justify-center shrink-0 border border-indigo-500/30">
            <ShieldCheck className="w-5 h-5" />
          </div>
          <div>
            <h4 className="font-bold text-slate-100 text-xs">Secure Client Isolation Active</h4>
            <p className="text-[11px] text-slate-400 mt-0.5">
              Filtered exclusively for <strong className="text-slate-200">Acme Corp</strong> via Custom Claims & Firestore Path Rules.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4 flex items-center justify-between border-slate-800/80">
          <div>
            ...
            <p className="text-xl font-bold text-slate-100 mt-1">{projects.length}</p>
          </div>
        ...
        <Card className="p-4 flex items-center justify-between border-slate-800/80">
          <div>
            ...
            <p className="text-xl font-bold text-amber-400 mt-1">{pendingApprovals.length}</p>
          </div>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-slate-100">Deliverable Sign-offs Needed</h3>
        {approvals.map((app) => (
          <Card key={app.approvalId} hover className="flex items-center justify-between p-4 border-slate-800">
            <div className="space-y-1">
              ...
              <span className="text-[10px] text-slate-500 block">Project: {app.projectName} • Requested: {app.requestedAt}</span>
            </div>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-slate-100">Project Progress Overview</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {projects.map((p) => (
            <Card key={p.projectId} hover className="space-y-3 border-slate-800">
